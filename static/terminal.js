document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log("ð BOOT SEQUENCE INITIATED...");
    setTimeout(() => {
    ...
    function updateVesselInvestmentAnalysis(tanker) {
        ...
        metricsPanel.innerHTML = `
            ...
            <div style="background: ${analysis.impact >= 0 ? 'rgba(239,68,68,0.15)' : 'rgba(16,185,129,0.15)'}; border: 2px solid ${analysis.impact >= 0 ? '#ef4444' : '#10b981'}; border-radius: 6px; padding: 14px; margin-bottom: 14px;">
                <div style="color: #9ca3af; font-size: 10px; margin-bottom: 6px; font-weight: 500;">ð° INVESTMENT IMPACT</div>
            ...
            <!-- Route & Status -->
            <div style="background: rgba(31,41,55,0.5); border: 1px solid #374151; border-radius: 6px; padding: 12px; margin-bottom: 14px;">
                <div style="color: #3b82f6; font-size: 11px; font-weight: 600; margin-bottom: 8px;">ð ROUTE DETAILS</div>
            ...
            <!-- Proof & Evidence -->
            <div style="background: rgba(31,41,55,0.5); border: 1px solid #374151; border-radius: 6px; padding: 12px; margin-bottom: 14px;">
                <div style="color: #fbbf24; font-size: 11px; font-weight: 600; margin-bottom: 8px;">ð PROOF & EVIDENCE</div>
                ${analysis.proof.map(item => `
                    <div style="padding: 6px 0; border-bottom: 1px solid #1f2937;">
                        <div style="color: #e5e7eb; font-size: 10px; font-weight: 600; margin-bottom: 2px;">â ${item.title}</div>
            ...
            <!-- Risk Assessment -->
            <div style="background: rgba(31,41,55,0.5); border: 1px solid #374151; border-radius: 6px; padding: 12px; margin-bottom: 14px;">
                <div style="color: #ef4444; font-size: 11px; font-weight: 600; margin-bottom: 8px;">â ï¸ RISK ASSESSMENT</div>
                ...
                ${analysis.risks.map(risk => `
                    <div style="padding: 4px 0; border-bottom: 1px solid #1f2937;">
                        <div style="color: #ef4444; font-size: 9px;">â¡ ${risk}</div>
                    </div>
                `).join('')}
            </div>
            ...
            <div style="background: linear-gradient(135deg, rgba(16,185,129,0.1), rgba(34,197,94,0.1)); border: 1px solid #10b981; border-radius: 6px; padding: 12px;">
                <div style="color: #10b981; font-size: 11px; font-weight: 600; margin-bottom: 8px;">ð¡ RECOMMENDATIONS</div>
                ${analysis.recommendations.map((rec, idx) => `
                    <div style="padding: 6px 0; ${idx < analysis.recommendations.length - 1 ? 'border-bottom: 1px solid #1f2937;' : ''}">
                        ...
                        <div style="color: #10b981; font-size: 9px; margin-top: 2px;">ð° ${rec.benefit}</div>
                    </div>
                `).join('')}
            </div>
        `;
    ...
    function renderStudyTopics(topics) {
        ...
        const categories = {
            'ANALYSIS': { name: 'Technical Analysis', icon: 'ð', color: '#10b981', desc: 'Charts, Patterns, Indicators' },
            'FUNDAMENTALS': { name: 'Fundamentals', icon: 'ð', color: '#3b82f6', desc: 'Financial Statements, Ratios' },
            'DERIVATIVES': { name: 'Derivatives', icon: 'ð', color: '#f59e0b', desc: 'Options, Futures, Swaps' },
            'STRATEGY': { name: 'Trading Strategy', icon: 'âï¸', color: '#8b5cf6', desc: 'Risk Mgmt, Portfolio Construction' }
        };
        ...
        content.innerHTML = `
            <div style="font-family: 'Inter', sans-serif;">
                ...
                <div style="background: linear-gradient(135deg, rgba(16,185,129,0.1), rgba(34,197,94,0.1)); 
                    ...
                    <div style="color: #10b981; font-size: 10px; font-weight: 600; margin-bottom: 8px;">â MODEL ANSWER</div>
                ...
                <div style="background: rgba(59,130,246,0.1); border: 1px solid #3b82f6; border-radius: 8px; padding: 16px;">
                    <div style="color: #3b82f6; font-size: 10px; font-weight: 600; margin-bottom: 8px;">ð¡ DETAILED EXPLANATION</div>
    ...
    function renderMarketScenario(scenario) {
        ...
        content.innerHTML = `
            <div style="font-family: 'Inter', sans-serif;">
                ...
                <div style="background: rgba(31,41,55,0.5); border: 1px solid #374151; border-radius: 8px; padding: 16px; margin-bottom: 16px;">
                    <div style="color: #f59e0b; font-size: 12px; font-weight: 600; margin-bottom: 12px;">ð CHALLENGE</div>
                    <div style="color: #e5e7eb; font-size: 13px; line-height: 1.7;">${scenario.challenge}</div>
                </div>
                
                <div style="background: rgba(31,41,55,0.5); border: 1px solid #374151; border-radius: 8px; padding: 16px; margin-bottom: 16px;">
                    <div style="color: #3b82f6; font-size: 12px; font-weight: 600; margin-bottom: 12px;">ð¡ HINTS</div>
                ...
                <div id="solutionContent" style="display: none; background: linear-gradient(135deg, rgba(16,185,129,0.1), rgba(34,197,94,0.1)); 
                    ...
                    <div style="color: #10b981; font-size: 12px; font-weight: 600; margin-bottom: 8px;">â SOLUTION APPROACH</div>
    ...
    function renderStudyResources(resources) {
        ...
        content.innerHTML = `
            <div style="font-family: 'Inter', sans-serif;">
                ...
                <h2 style="color: #10b981; font-size: 16px; margin-bottom: 16px;">ð Study Resources</h2>
                    ...
                    ${resources.map(r => `
                        <div style="background: rgba(31,41,55,0.5); border: 1px solid #374151; 
                            ...
                            <div style="display: flex; align-items: flex-start; gap: 12px;">
                                <div style="font-size: 24px;">${typeIcons[r.type] || 'ð'}</div>
                                <div style="flex: 1;">
    ...
    function handleResponse(data) {
        ...
        if (data.type === 'STUDY_VIEW') {
            let html = `
                ...
                <div style="margin-bottom: 20px;">
                    <div style="color: #3b82f6; font-weight: 600; margin-bottom: 10px; border-bottom: 1px solid #374151; padding-bottom: 5px;">ð° LIVE NEWS</div>
            `;
                        ...
                        <div style="background: #111827; border: 1px solid #1f2937; border-radius: 6px; padding: 10px; margin-bottom: 8px;">
                            <div style="display: flex; justify-content: space-between; align-items: start;">
                                <span style="color: ${sentimentColor}; font-size: 10px; font-weight: 600;">
                                    ${item.sentiment === 'BULLISH' ? 'ð¢' : item.sentiment === 'BEARISH' ? 'ð´' : 'âª'} ${item.sentiment}
                                </span>
            ...
            html += `
                ...
                <div style="margin-bottom: 20px;">
                    <div style="color: #8b5cf6; font-weight: 600; margin-bottom: 10px; border-bottom: 1px solid #374151; padding-bottom: 5px;">ð LEARNING RESOURCES</div>
            `;
            ...
            html += `
                ...
                <div style="background: #111827; border: 1px solid #374151; border-radius: 6px; padding: 10px; text-align: center;">
                    <span style="color: #6b7280; font-size: 10px;">ð ${data.glossary_count} terms in glossary</span>
        ...
        if (data.type === 'MOVERS_VIEW') {
            ...
            // Gainers
            html += '<div style="color: #10b981; font-weight: 600; margin-bottom: 8px; font-size: 11px;">ð¢ TOP GAINERS</div>';
            if (data.gainers) {
            ...
            // Losers
            html += '<div style="color: #ef4444; font-weight: 600; margin: 12px 0 8px; font-size: 11px;">ð´ TOP LOSERS</div>';
            if (data.losers) {
                    ...
                    html += `
                        <div style="background: ${isToday ? 'rgba(245,158,11,0.15)' : '#111827'}; border: 1px solid ${isToday ? '#f59e0b' : '#1f2937'}; border-radius: 6px; padding: 12px; margin-bottom: 8px;">
                            ...
                            <div style="display: flex; justify-content: space-between; margin-top: 8px;">
                                <span style="color: #6b7280; font-size: 10px;">ð ${e.formatted_date} (${e.day_name}) â¢ ${e.time}</span>
                                <span style="color: #3b82f6; font-size: 10px;">${e.currency}</span>
                            </div>
                            ${isToday ? '<div style="color: #f59e0b; font-size: 9px; margin-top: 6px; font-weight: 600;">â¡ TODAY</div>' : ''}
                        </div>
                    `;
                });
            } else {
...
function renderStudyNews(newsItems) {
    ...
    const newsHTML = newsItems.map(item => `
            <div style="padding: 10px; border-bottom: 1px solid #222; margin-bottom: 5px;">
                <div style="color: #10b981; font-size: 10px; font-weight:700;">${item.source} â¬ ¢ ${item.published}</div>
                <div style="color: #eee; font-size: 14px; margin: 4px 0;">
                    <a href="${item.link}" target="_blank" style="color: #eee; text-decoration: none;">${item.title}</a>
                </div>
